...
export default class EmailFormItem {

    error = true
    builderFormData = {
        type: FORM_ITEM_TYPE.EMAIL.key,
        props:{
            label: '',
            required: false,
            placeholder: ''
    ...
    getObjectFromJSON = jsonObj => {
        this.builderFormData.type = FORM_ITEM_TYPE.EMAIL.key
        ...
        let builderView = props => {
            ...
            return( 
                <div className={ `${(activeIndex==index) ? 'input-builder-view-active' : 'input-builder-view'} ${this.error ? 'error-case' : ''}`} data-type={index}>
                    <label  data-type={index}>{this.label || 'Email'}</label><br/>
